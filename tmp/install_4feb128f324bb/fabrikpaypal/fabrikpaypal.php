...
class FabrikModelfabrikpaypal extends FabrikModelFormPlugin {
	...
	protected function getAccountEmail(&$params)
	{
		if ($params->get('paypal_testmode') == 1) {
			$email = $params->get('paypal_sandbox_accountemail');
		} else {
...
echo "<pre>";print_r($opts);exit;
	...
	function ipn()
	{
						...
						if ($status == 'ok') {
							...
							if ($ipn !== false) {
								...
								if (method_exists($ipn, $ipn_function)) {
									$status = $ipn->$ipn_function($tableModel, $request, $set_list, $err_msg);
									if ($status != 'ok') {
										break;
									}
								}
								...
								if (method_exists($ipn, $txn_type_function)) {
									$status = $ipn->$txn_type_function($tableModel, $request, $set_list, $err_msg);
									if ($status != 'ok') {
										break;
									}
								}
		...
		if ($status != 'ok') {
			...
			// $$$ rob there are a whole load of transaction types that aren't payments - lets not send people emails informing them they've made a payment !
			if ($send_default_email == '1' && !in_array($txn_type, array('subscr_cancel', 'subscr_eot', 'subscr_failed', 'subscr_modify'))) {
