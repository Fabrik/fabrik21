...
require_once(COM_FABRIK_FRONTEND.DS.'plugins'.DS.'form'.DS.'fabrikpaypal'.DS.'scripts'.DS.'fabrikar_subs.php');

$db =& JFactory::getDBO();

expireNonRecurringSubs();
...
signup_date, ".$db->Quote($sitename)." AS sitename, ".$db->Quote($mailfrom)." AS mailfrom, ".$db->Quote($url)." AS url, ".$db->Quote($fromname)." AS fromname, s.recurring,
CASE
...
foreach ($res as $row) {
	...
	if (array_key_exists($row->daysleft, $expiration_mails) && $row->recurring == 0) {
		...
		// need to test if they have already renewed! (for now just adding a notice into the email!)
...
function expireNonRecurringSubs(){
	...
	JUtility::sendMail($mailfrom, $fromname, 'rob@pollen-8.co.uk', 'Expired '.JURI::base().' subs:', $report, true);
}