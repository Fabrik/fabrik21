...
class FabrikModelFabrikCalc extends FabrikModelElement {

	var $_pluginName = 'calc';
	...
	{
		// @TODO - need to fix issue when coming from getFilterArray, where $data is null
		$w = new FabrikWorker();
	...
	public function preProcess($c)
	{
		...
		$group =& $this->getGroup();
		$this_joinid = $group->getGroup()->join_id;

		foreach ($joindata as $joinid => $thisJoindata) {
			if ($joinid == $this_joinid) {
				foreach ($thisJoindata as $key => $val) {
					$long_key = 'join___' . $this_joinid . '___' . $key;
	...
	function renderTableData_csv($data, $thisRow)
	{
	...
	{
		FabrikHelperHTML::script('javascript.js', 'components/com_fabrik/plugins/element/fabrikcalc/', false);
	}
	...
	function elementJavascript($repeatCounter)
	{
		...
		$params =& $this->getParams();
		$calc = $params->get('calc_calculation');
		// $$$ hugh - added defensive coding to remove single quotes, as some folk take the tooltip too literally
		// and put '{foo__bar},{foo___garply}' i.e. with quotes, in their ajax observe field!
		$obs = str_replace("'", "", $params->get('calc_ajax_observe'));
		$obs = empty($obs) ? array() : explode(',', str_replace("'", "", $params->get('calc_ajax_observe')));
	...
	protected function getSumQuery(&$tableModel, $label = "'calc'" )
	{
		...
		if ($fields[$this->getElement()->name]->Type == 'time') {
			...
			return parent::getSumQuery($tableModel, $label);
		}
	}
	...
	*/
	...
	{
		// as we don't know wtf the user will be doing with a calc,
		// just include lightbox and be done with it.
		return true;
	}
}
?>