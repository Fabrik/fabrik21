...
class FabrikModelFabrikKaltura extends FabrikModelElement {

	var $_pluginName = 'kaltura';
	...
	function renderTableData($data, $oAllRowsData)
	{
		...
		FabrikHelperHTML::script('swfobject.js', 'media/com_fabrik/js/', false);
...
return '<div id="'.$id.'"></div>';
	...
	private function getKalturaFlashVars()
	{
		if (!isset($this->kalturaFlashVars)) {
			...
			$flashVars["ks"] 		= $ksession["result"]["ks"];
			$flashVars["kshowId"] 		= -2;
	...
	$("'.$id.'").value = res.join("'.GROUPSPLITTER2.'");
}');
	...
	{
		FabrikHelperHTML::script('swfobject.js', 'media/com_fabrik/js/', false);
		FabrikHelperHTML::script('javascript.js', 'components/com_fabrik/plugins/element/fabrikkaltura/', true);
	}
