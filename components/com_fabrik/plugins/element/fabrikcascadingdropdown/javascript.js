 ...
 */
 ...
 var fbCascadingdropdown = FbElement.extend({
	initialize: function(element, options) {
		...
		this.doWatchEvent = this.dowatch.bindAsEventListener(this);
	...
	change:function(v, triggerid)
	{
		...
		if(window.ie) {
			...
			// this is the original cdd element
				...
				if(s == '_' && $type(parseInt(i)) == 'number' && i != '0') {
		...
		var url = this.options.liveSite + 'index.php?option=com_fabrik&format=raw&controller=plugin&task=pluginAjax&plugin=fabrikcascadingdropdown&method=ajax_getOptions&element_id=' + this.options.id;
		...
		var data = $extend(formdata, {'v':v, 'formid':this.form.id, 'fabrik_cascade_ajax_update':1});
		if (this.myAjax) {
			// $$$ rob stops ascyro behaviour when older ajax call might take longer than new call and thus populate the dd with old data.
			this.myAjax.cancel();
		}
		this.myAjax = new Ajax(url, {method:'post', 'data': data,
		onComplete: this.changeComplete.bindAsEventListener(this)}).request();
	},
	
	changeComplete: function(json) {
		...
		json.each(function(item, k) {
			...
			if(this.options.showDesc === true && item.description) {
				...
				new Element('div', {styles:{display:'none'}, 'class':classname}).setHTML(item.description).injectInside(c);
			}
		}.bind(this));
		...
		// $$$ rob removed this.dowatch_event in favour of new Event.mock()
		...
		if (!this.ignoreAjax) {
			...
			// $$$ hugh - will cause skipValidation() to return true, as we don't want to validate
			// when change is because of reloading after a watched element change.  Will be set
			// back to false in skipValidation(), which gets called back from validation code in form.js.
			this.skip_validation = true;
			this.element.fireEvent('change', new Event.Mock(this.element, 'change'));
			// $$$ hugh - hey Rob, setting ignoreShowDesc back to false here ... won't that happen before
			// async 'change' event runs?  See comments above about skip_validation.
			this.ingoreShowDesc = false;
		}
		this.ignoreAjax = false;
	...
	cloned: function(c) {
		...
		if($(this.options.watch)) {
			...
			if($(this.options.watch)) {
				...
				// $$$ hugh - hopefully, this is now done by default in the main element code
				// so all elements now remove any 'change' events which weren't added  through addNewEvent on page load
