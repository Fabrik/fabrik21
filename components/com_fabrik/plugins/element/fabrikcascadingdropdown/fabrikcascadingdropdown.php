...
class FabrikModelFabrikCascadingdropdown  extends FabrikModelFabrikDatabasejoin {

	var $_pluginName = 'fabrikcascadingdropdown';
...
<div id="page-<?php echo $this->_name;?>" class="elementSettings"
	...
	if ($element->id == 0) {
		echo JText::_("available on save");
	} else {
		echo $pluginParams->render('params');
		?>
<fieldset><legend><?php echo JText::_('Watch element') ?></legend> <?php echo $pluginParams->render('params', 'watch'); ?>
</fieldset>
<fieldset><legend><?php echo JText::_('Advanced') ?></legend> <?php echo $pluginParams->render('params', 'advanced'); ?>
</fieldset>
<fieldset><legend><?php echo JText::_('Front end') ?></legend> <?php echo $pluginParams->render('params', 'frontend'); ?>
</fieldset>
		<?php
	}
	?></div>
	...
	{
		FabrikHelperHTML::script('javascript.js', 'components/com_fabrik/plugins/element/fabrikcascadingdropdown/', false);
	}
	/**
	...
	function render($data, $repeatCounter = 0)
	{
		...
		if (!$this->_editable) {
			if ($params->get('cascadingdropdown_readonly_link') == 1) {
				$tableid = (int)$params->get('cascadingdropdown_table');
				if ($tableid  !== 0) {
					$db->setQuery("SELECT form_id FROM #__fabrik_tables WHERE id = $tableid");
					...
					$defaultLabel = $this->_replaceWithIcons($defaultLabel);
		...
		if ($params->get('cdd_desc_column') !== '') {
			...
			for ($i=0; $i < count($this->_optionVals); $i++) {
				...
				$v = isset($opt->value) ? $opt->value : '';
				$display = $v == $default ? '' : 'none';
				$c = $i+1;
				$desc = isset($opt->description) ? $opt->description : '';
				$str .= "<div style=\"display:$display\" class=\"notice description-".$c."\">".$desc."</div>";
			}
			$str .= "</div>";
		}
	...
	function ajax_getOptions()
	{
		...
		$this->_table =& $this->_form->getTableModel();
	...
	function _getWatchFullName()
	{
		...
		/*
		 $params =& $this->getParams();
		 $watch = $params->get('cascadingdropdown_observe');
		 $elementModel =& JModel::getInstance('element', 'FabrikModel');
		 $elementModel->setId($watch);
		 */
	...
	private function _getWatchElement()
	{
		if (!isset($this->watchElement)) {
			$params =& $this->getParams();
			$watch = $params->get('cascadingdropdown_observe');
			$this->watchElement =& JModel::getInstance('element', 'FabrikModel');
			$this->watchElement->setId($watch);
		}
	...
	function _buildQuery($data = array(), $repeatCounter = 0)
	{
		...
		$wherewhen = $params->get('cascadingdropdown_where_when', '3');
		...
		//$this->_sql[$repeatCounter] = "SELECT DISTINCT($key) AS value, $val AS text FROM ".$db->nameQuote($table) .' AS '.$db->nameQuote($join->table_join_alias)." $where " . $tableModel->_buildQueryJoin() . " ";
		$sql = "SELECT DISTINCT($key) AS value, $val AS text";
		$desc = $params->get('cdd_desc_column', '');
		if ($desc !== '') {
			// $$$ hugh - need to do the table alias name swap thing!
			$desc = FabrikString::safeColName($desc);
			$desc = preg_replace("#^`($table)`\.#", "`{$join->table_join_alias}`.", $desc);
	...
	{
		return 'cascadingdropdown_connection';
	}

	/**
	 * $$$ rob DERPECIATED - NOW JUST SET connectionParam() and use dbjoins' _loadConnection() method
	 * @access protected
	...
	/*protected function &_loadConnection()
	 {
		...
		$id = $params->get('cascadingdropdown_connection', $cid);
		if ($cid == $id) {
	...
	private function getDbName()
	{
		if (!isset($this->_dbname) || $this->_dbname == '') {
			...
			$db =& JFactory::getDBO();
			$db->setQuery("SELECT db_table_name FROM #__fabrik_tables WHERE id = ".(int)$id);
	...
	function getFilter($counter = 0, $normal = true)
	{
		...
		//$fabrikDb->setQuery("SELECT db_table_name FROM #__fabrik_tables WHERE id = {$params->get('cascadingdropdown_table')}");
		...
		/*$watch = $this->_getWatchFullName();
		 ...
		 if (isset($tableModel->filters[$watch])) {
			$whereval = $tableModel->filters[$watch]['value'];
			}
	...
	function _filterJS($normal, $container)
	{
		...
		"window.addEvent('domready', function() { new FabCddAutocomplete('$htmlid', $opts); });"
			);
		}
	...
	function onSave()
	{
		...
		$join->attribs = "join-label=" . $join_label . "\n";
		$join->store();
		return true;
	}
