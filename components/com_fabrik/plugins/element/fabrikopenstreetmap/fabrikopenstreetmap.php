...
class FabrikModelFabrikOpenstreetmap extends FabrikModelElement {

	var $_pluginName = 'openstreetmap';
	...
	function renderTableData($data, $oAllRowsData)
	{
		...
		if (strstr($data, GROUPSPLITTER)) {
			...
			$str .= $this->_staticMap( $data, $w, $h, $z);
		}
		return $str;
	}
	...
	function formJavascriptClass()
	{
		...
		if (!isset($jsloaded)) {
			...
			FabrikHelperHTML::script('javascript.js', 'components/com_fabrik/plugins/element/fabrikopenstreetmap/', true);
			FabrikHelperHTML::script('openlayers_ext.js', 'components/com_fabrik/libs/openlayers/', true);
	...
	function _staticMap( $v, $w=null, $h=null, $z=null, $repeatCounter = 0)
	{
		...
		$opts->defaultLayer = $params->get('fb_osm_defaultlayer');
		$opts = json_encode($opts);
		FabrikHelperHTML::addScriptDeclaration("window.addEvent('load', function() {new FbOpenStreetMap('$id', $opts);});");
		return $str;
	}
	/**
