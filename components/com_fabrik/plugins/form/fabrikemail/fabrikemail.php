	...
	function onAfterProcess($params, &$formModel )
	{
		...
		if (JFile::exists($emailTemplate)) {
			if (JFile::getExt($emailTemplate) == 'php') {
				...
				// $$$ hugh - added ability for PHP script to abort
				// essentially same as if 'condition' was false;
				if ($message === false) {
					return;
				}
			} else {
		...
		$editURL = COM_FABRIK_LIVESITE . "index.php?option=com_fabrik&amp;view=form&amp;fabrik=".$formModel->getId()."&amp;rowid=".JRequest::getVar('rowid');
		$viewURL = COM_FABRIK_LIVESITE . "index.php?option=com_fabrik&amp;view=details&amp;fabrik=".$formModel->getId()."&amp;rowid=".JRequest::getVar('rowid');
