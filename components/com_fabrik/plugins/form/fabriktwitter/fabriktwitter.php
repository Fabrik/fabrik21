...
class FabrikModelfabriktwitter extends FabrikModelFormPlugin {
	...
	function sendTweet($params, $connection)
	{
		...
		if ($content->remaining_hits <= 0) {
			JError::raiseNotice(500, JText::_('PLG_FORM_TWITTER_ACCOUNT_LIMIT_REACHED'));
		}
		...
		switch ($connection->http_code) {
			...
			case '304':
				...
				break;
			default:
				JError::raiseNotice(JText::_('PLG_FORM_TWITTER_ERROR_MSG'), "$connection->http_code : $status->error");
		}
	...
	public function updateAdmin()
	{
		...
		$row->attribs = $params->updateAttribsFromParams($opts);
	...
	function renderAdminSettings($elementId, &$row, &$params, $lists, $c)
	{
		...
		. $params->render('params', 'signin', false, $c )
		.'</div>';
