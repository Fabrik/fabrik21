...
class fabrikModelGooglemap extends FabrikModelVisualization {
	...
	function getJs()
	{
		...
		$opts->polygonopacity = $params->get('fb_gm_polygon_fillOpacity', array(), '_default', 'array');
		$opts->polygonfillcolour = $params->get('fb_gm_polygon_fillColor', array(), '_default', 'array');
		$opts->use_polygon = $params->get('fb_gm_use_polygon');
		...
		foreach ($aTables as $tableid) {
			...
			foreach ($data as $groupKey => $group) {
				foreach ($group as $row) {
					...
					if (!array_key_exists($coordColumn, JArrayHelper::fromObject($row))) {
						if (!$missingFieldNotice) {
							JError::raiseNotice(500, $coordColumn . ' not found - can not render icon');
							$missingFieldNotice = true;
						}
						continue;
					}
