...
/**
 * Add a radius search option to the table filters
 * @package Joomla
 * @subpackage Fabrik
 * @author Rob Clayburn
...
class FabrikModelRadius_search extends FabrikModelTablePlugin
{
	...
	function onMakeFilters(&$params, &$model)
	{
		...
		<table>\n<tbody>\n<tr>\n<td><label for=\"radius_search_lat\">".JText::_('LATITUDE')."</label></td>\n<td><input name=\"radius_search_lat\" value=\"$lat\" id=\"radius_search_lat\" $class size=\"6\"/></td>\n</tr>\n
		<tr>\n<td><label for=\"radius_search_lon\">".JText::_('LONGITUDE')."</label></td>\n<td><input name=\"radius_search_lon\" value=\"$lon\" id=\"radius_search_lon\" $class size=\"6\"/></td>\n</tr>\n</tbody>\n</table>\n</div>";

		$strSlider = $this->slider();

		$checked = $type[0] == 'mylocation' ? 'checked="checked"' : '';
		//$options = "<label>".JText::_('MY_LOCATION')."<input type=\"radio\" name=\"radius_search_type[]\" value=\"mylocation\" $checked /></label><br />";
		$options = "<label><input type=\"radio\" name=\"radius_search_type[]\" value=\"mylocation\" $checked />".JText::_('MY_LOCATION')."</label><br />";
		...
		$options .= "<label><input type=\"radio\" name=\"radius_search_type[]\" value=\"place\" $checked />".strip_tags($placeElement->label)."</label><br />";
		$checked = $type[0] == 'latlon' ? 'checked="checked"' : '';
		//$options .= "<label>".JText::_('COORDINATES')."<input type=\"radio\" name=\"radius_search_type[]\" value=\"latlon\" $checked /></label><br />";
		$options .= "<label><input type=\"radio\" name=\"radius_search_type[]\" value=\"latlon\" $checked />".JText::_('COORDINATES')."</label><br />";
		...
		<label>".JText::_('ACTIVE')."<input type=\"radio\" $yessel name=\"radius_search_active[]\" value=\"1\" /></label>
		<label>".JText::_('INACTIVE')."<input type=\"radio\" $nosel name=\"radius_search_active[]\" value=\"0\" /></label>
		<div class=\"radius_search_options\">
		<table class=\"radius_table\" style=\"width:100%\">
		<tbody>
			<tr>
				<td>".JText::_('DISTANCE')."</td>
				<td>$strSlider</td>
			</tr>
			<tr>
				<td>" . JText::_('FROM') . ":<br />$options</td>
				<td style=\"text-align:left\">$strPlace $strLatLon </td>
			</tr>
			</tbody>
		</table>
		</div>
		...
		$doc->addStyleDeclaration("table.radius_table{border-collapse:collapse;border:0;}
	...
	/**
	 * this is used to put the radius search data into the tablefilter model
	 ...
	 * @param array filters created from tablefilter::getPostFilters();
	 */
	...
	private function getMapElement()
	{
		...
		$params =& $this->getParams();
		$this->mapElement = JArrayHelper::getValue($elements, $params->get('radius_mapelement'));
		return $this->mapElement;
	}
	...
	function loadJavascriptInstance($params, $model, $args)
	{
		...
		$lang = new stdClass();
		$lang->no_geoloc = JText::_('NO_GEOLOCATION_AVAILABLE');
		$lang = json_encode($lang);
		$this->jsInstance = "new FabRadiusSearch('$form_id', $opts, $lang)";
		return true;
	}
