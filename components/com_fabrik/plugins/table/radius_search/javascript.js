...
var FabRadiusSearch = FbTablePlugin.extend({
	initialize: function(element, options, lang) {
		...
		window.addEvent('domready', function() {
			...
			if ($type(this.element.getElement('.radius_search_options')) === false) {
				return;
			}
			this.fx = new Fx.Slide(this.element.getElement('.radius_search_options'));
			this.element.getElements('input[name^=radius_search_active]').addEvent('click', this.toggleActive.bindAsEventListener(this));
			...
			this.element.getElements('input[name^=radius_search_type]').addEvent('click', this.toggleFields.bindAsEventListener(this));
			...
			this.mySlide = new Slider(this.element.getElement('.fabrikslider-line'), this.element.getElement('.knob'), {
				onChange: function(pos) {
					output.value = pos;
					output2.setText(pos + this.options.unit);
				}.bind(this),
			...
			if(geo_position_js.init()) {
				geo_position_js.getCurrentPosition(this.setGeoCenter.bind(this),this.geoCenterErr.bind(this),{enableHighAccuracy:true});
			}
