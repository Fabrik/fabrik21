...
var fbTableChown = FbTablePlugin.extend({
	mochaWindow: null,
	iframeform: null,
	...
	watchPluginButton:function() {
		...
		var ok = false;
		var url = this.options.liveSite + "index.php?option=com_fabrik&controller=table.chown&tmpl=component&id="+this.tableid+"&renderOrder="+this.options.renderOrder;
	...
	watchSubmit:function(warning) {
		var err = '';
		if (this.iframeform.chown_to_val.value == '' || this.iframeform.chown_to_val.value == '0') {
			err += (warning);
		}
		if (err != '') {
			alert(err);
