...
class fabrikModelAddmany extends FabrikModelTablePlugin {

	var $_buttonPrefix = 'addmany';
	...
	function canSelectRows()
	{
		...
		$access = $params->get('addmany_access');
	...
	function button_result()
	{
		...
		if ($this->canUse()) {
			$name = $this->_getButtonName();
			return '<input type="button" name="'.$name.'" value="' . $params->get('addmany_button_label') . '" class="button tableplugin" onclick="oTable'.$this->model->_id.'.plugins[' . $this->renderOrder . '].watchPluginButton()"/>';
		}
	}
	...
	function loadJavascriptClass()
	{
		...
		FabrikHelperHTML::script('javascript.js', 'components/com_fabrik/plugins/table/addmany/', false);
	}
	...
	function loadJavascriptInstance($params, $model, $args)
	{
		...
		$opts = new stdClass();
		$opts->window_title = $params->get('addmany_button_label');
		$opts->window_width = (int)$params->get('addmany_window_width', 300);
		$opts->window_height = (int)$params->get('addmany_window_height', 100);
		$opts->liveSite	= COM_FABRIK_LIVESITE;
		$opts->renderOrder = $this->renderOrder;
		$this->model =& $model;
		$opts->mooversion = ($fbConfig->get('usefabrik_mootools', false)) ? 1.2 : 1.1;
		$opts = json_encode($opts);
		$this->jsInstance = "new fbTableAddmany('$form_id', $opts)";
		return true;
	}
}
