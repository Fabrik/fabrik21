...
class FabrikModelElement extends JModel {
   ...
   * @since 2.1.1
	...
	function getDefaultFilterVal($normal = true, $counter = 0)
	{
		...
		// $$$ hugh - for now, skip this chunk of code if !$normal, ie. we are dealing with advanced filters
		// as it makes no sense ... it tries to use $counter (the sequence number of the advanced
		// search row being built) as an index into $filters array, and the two are totally unrelated.  See:
		// http://fabrikar.com/forums/showthread.php?t=19679
		if ($default == '' && $normal) {
			if (empty($filters)) {
				return '';
			}
	...
	{
		// $$$ hugh - adding testing for $drop.
		// Perhaps debateable, but if they said "No" to "drop this column",
		// then for now let's not drop any repeat element tables.
		if ($drop && $this->isJoin()) {
			$tableModel =& $this->getTableModel();
			$db =& $tableModel->getDb();
...
FROM (SELECT DISTINCT $table->db_primary_key, $name AS value, $label AS label FROM ".FabrikString::safeColName($table->db_table_name)." $joinSQL $whereSQL) AS t";
	...
	/**
	 * Return an element's value suitably formatted for turning in to a QR code.
	 * Default is just to return as-is, but some elements (like Google map, or link)
	 * will need to override this and massage their value into something
	 * that makes sense for QR encoding ca
	 * Enter description here ...
	 * @param unknown_type $value current value of element
	 * @param unknown_type $data rest of form / row data
	 * @param unknown_type $repeatCounter
	 * @param unknown_type $view is this a table or a form
	 */
	function getQRValue($value, $data = array(), $repeatCounter = 0, $view = 'table')
	{
		if ($this->_inRepeatGroup) {
			$val = array();
			foreach ($value as $v2) {
				$val[] = $this->_getQRValue($v2, $data, $repeatCounter, $view);
			}
		} else {
			$val = $this->_getQRValue($value, $data, $repeatCounter, $view);
		}
		return $val;
	}
...
count_split=";
	...
	{
		// $$$ until we sort it out, we can't do search all on repeat elements
		if ($this->isJoin()) {
			return false;
		}
