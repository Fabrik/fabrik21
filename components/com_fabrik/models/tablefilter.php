...
class FabrikModelTablefilter extends JModel {
	...
	{
		return JRequest::getInt('id') == JRequest::getInt('activetableid') || JRequest::getInt('activetableid') === 0;
	}
	...
	private function getSearchAllFilters(&$filters)
	{
		...
		$key = $this->getSearchAllSessionKey();
	...
	*/
	...
	public function clearFilters()
	{
		...
		$fromFormId = $this->getFormFormSessionId();
		if ($fromFormId != $this->tableModel->getFormModel()->get('id')) {
			$app->setUserState($this->getFormSearhAllKey(), '');
		}
	}
	
	/**
	 * get the key used to store the search all value in the session
	 * @return	string	session key
	 */
	
	public function getSearchAllSessionKey()
	{
		$id = $this->tableModel->getId();
		return 'com_fabrik.list' . $id . '.filter.searchall';
	}
	
	public function getFormSearhAllKey()
	{
		$id = $this->getFormFormSessionId();
		return 'com_fabrik.searchform.form' . $id . '.searchall';
	}
	
	public function getFormFormSessionId()
	{
		$app = JFactory::getApplication();
