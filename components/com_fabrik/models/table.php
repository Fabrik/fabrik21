...
class FabrikModelTable extends JModel {
	...
	/** @var force formatData() to format all elements, uses formatAll() accessor method **/
	var $_format_all = false;
	...
	function getCustomJsAction()
	{
		if (file_exists(COM_FABRIK_FRONTEND.DS.'js'.DS.'table_'.$this->getId().".js")) {
			FabrikHelperHTML::script('table_' . $this->getId() . ".js", 'components/com_fabrik/js/');
		}
	}
	...
	*/
	...
	function getData()
	{
		...
		$pluginManager->runPlugins('onLoadDataBeforeFormat', $this, 'table');
	...
	function formatData(&$data)
	{
		...
		foreach ($groups as $groupModel) {
			...
			// $$$ hugh added formatAll() mechanism, so things like table email plugin can force us to format all data
	...
	function viewDataLink($popUp = false, $element, $row = null, $key = '', $val = '', $count = 0, $f)
	{
		...
		$toreturn = '<div style="text-align:center; vertical-align:middle;">';
		if (($count === 0 && (int)$params->get('show_related_add', 0) === 2) or ((int)$params->get('show_related_add', 0) === 1)) {
			$aExisitngLinkedForms = $params->get('linkedform', '', '_default', 'array');
			$linkedForm = JArrayHelper::getValue($aExisitngLinkedForms, $f, false);
		...
		if (((int)$params->get('show_related_info', 0) == 0) or (((int)$params->get('show_related_info', 0) == 2)  and ($count))){
			if ($popUp) {
				FabrikHelperHTML::mocha('a.popupwin');
				$opts = new stdClass();
				$opts->maximizable = 1;
			...
			return $url.' '.$toreturn.'</div>';
		} else return $toreturn.'</div>';
	}
	...
	private function _groupFilterSQL(&$filters, $type = '*')
	{
		...
		while (list($vkey, $i) = each($vkeys)) {
			...
			if ($filters['origvalue'][$i] != '' || $filters['sqlCond'][$i] != '') {
				// $$$ hugh - testing changing these two lines to use $i, not $n
					...
					if ($filters['grouped_to_previous'][$n] == 1) {
						if (!$ingroup) {
							...
							if ($last_i && JArrayHelper::getValue($filters['search_type'], $last_i) == 'prefilter' && JArrayHelper::getValue($filters['search_type'], $i) !== 'prefilter') {
								...
								// $$$ hugh - ditto for searchall
								if ($filters['search_type'][$i] == 'searchall') {
									$gstart = "(";
									$groupedCount++;
								}
							} else {
	...
	{
		$canDelete = $this->getPluginManager()->runPlugins('onCanDelete', $this, 'table', $row);
		if (in_array(false, $canDelete)) {
			return false;
		}
	...
	function canCSVImport()
	{
		...
		if (!is_object($this->_access) || !array_key_exists('csvimport', $this->_access)) {
			if ($params->get('csv_import_frontend', 0) == 1) {
				//compat with old yes/no (cant do anything about no option as its the same as 'everybody'
	...
	function canCSVExport()
	{
		...
		if (!is_object($this->_access) || !array_key_exists('csvexport', $this->_access)) {
			if ($params->get('csv_export_frontend', 0) == 1) {
				//compat with old yes/no (cant do anything about no option as its the same as 'everybody'
	...
	function &getForm()
	{
		// $$$ hugh - added isset() because for some reason, _oForm does not exist when coming from J! search plugin
		if (!isset($this->_oForm) || is_null($this->_oForm)) {
	...
	function &getFilterArray()
	{
		...
		foreach ($this->filters['key'] as $i => $keyval) {
			...
			// $$$ hugh - still dicking around trying to get quoting and escapign right!
			// http://fabrikar.com/forums/showthread.php?t=22853
	...
	function &makeFilters($container = 'tableform_1', $type = 'table', $id = '')
	{
		...
		if ($params->get('search-mode', 'AND') == 'OR') {
			...
			$key = $filterModel->getSearchAllSessionKey();
			$v = $app->getUserStateFromRequest($key, 'fabrik_table_filter_all');
			if (trim($v) == '') {
				$fromFormId = $filterModel->getFormFormSessionId();
				if ($fromFormId != $this->getFormModel()->getId()) {
					$v = $app->getUserState($filterModel->getFormSearhAllKey());
				}
			}
	...
	function getHeadings()
	{
		...
		foreach ($groups as $groupModel) {
			...
			if (empty($groupHeadingKey)) {
				$groupHeadingKey = uniqid("empty_group_label_");
			}
			...
			foreach ($elementModels as $key => $elementModel) {
				...
				// $$$ hugh - adding cell class for group
				$group_class = "fabrik_list_" . $this->getId() . "_group_" . $groupModel->getId();
				...
				$headingClass[$key] = "class=\"fabrik_ordercell $group_class {$key}_heading {$element->id}_order " . $elementParams->get('tablecss_header_class')."\"";
				...
				$cellClass[$key] = "class=\"fabrik_row___{$key} fabrik_element $group_class " . $elementParams->get('tablecss_cell_class')."\"";
				...
				if (($isJoin && $groupModel->isJoin()) || (!$isJoin && !$groupModel->isJoin())) {
					...
					foreach ($elementModels as $elementModel) {
						...
						if ($elementModel->recordInDatabase($data)) {
							// $$$ hugh added the $postkey test for corner case of copying row with upload elements
							// http://fabrikar.com/forums/showthread.php?t=20591&page=2
							//if (array_key_exists($key, $data) && !in_array($key, $noRepeatFields)) {
							if ((array_key_exists($key, $data) || array_key_exists($postkey, $data)) && !in_array($key, $noRepeatFields)) {
								$noRepeatFields[] = $key;
								$lastKey = $key;
			...
			if (($isJoin && $groupModel->isJoin()) || (!$isJoin && !$groupModel->isJoin())) {
				...
				foreach ($elementModels as $elementModel) {
					...
					if (!$elementModel->canUse()) {
						$element =& $elementModel->getElement();
						$fullkey = $elementModel->getFullName(false, true, false);
						// $$$ rob 24/01/2012 if a previous joined data set had a ro element then if we werent checking that group is the
						...
						// if a joined group's foreign key has been set to an access level higher than the current user
						// then we don't want to swap it for the orig data which may be blank.
						// Its value has already been programatically set in the formModels processForm code.
						$foreignKey = $isJoin ? $groupModel->getJoinModel()->getJoin()->table_join_key : null;
						if ($key == $foreignKey)
						{
							continue;
						}
...
require-filter=0";
	...
	function getRow($id, $format = false, $loadJoin = false)
	{
		...
		if (!$loadJoin) {
			if ($format == true) {
				...
				$row = array($row);
				$this->formatData($row);
				$row = $row[FabrikWorker::firstKey($row)];
				$this->rows[$sig] = empty($row) ? new stdClass() : $row[0];
			} else {
			...
			$this->rows[$sig] = empty($rows) ? new stdClass() : $rows[0];
		}
		return $this->rows[$sig];
	}
	...
	*/
	// $$$ hugh - trying modified version of this method to fix custom add url, see ...
	// http://fabrikar.com/forums/showthread.php?p=122475#post122475
	function getAddRecordLink()
	{
		$qs = array();
		global $Itemid;
	...
	function getAddRecordLabel() {
		$params =& $this->getParams();
		$custom_add_label = $params->get('addlabel', '');
		return empty($custom_add_label) ? JText::_('ADD') : $custom_add_label;
	}
