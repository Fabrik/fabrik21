...
MUI.Window = new Class({
	...
	*/
	insertWindowElements: function() {
		...
		var id = options.id;
		if (navigator.userAgent.contains('Trident/5.0')) {
			MUI.ieSupport = '';
		}
		var cache = {};
...
MUI.extend({
	...
	*/
	...
	closingJobs: function(windowEl) {
		...
		// $$$ hugh - for whatever reason, we're often finding that instance doesn't
		// exist in MUI.instances.  So changing this to get instance from windowEL, which
		// seems to be how the only function that calls us (closeWindow) us gets the instance anyway.
		var instances = MUI.Windows.instances;
